  ...
  <div class="topbar">
    ...
    >
      <template v-for="item in menuList">
        <template v-if="item.subs && !item.noPermission">
          <el-submenu :class="[item.activeClass]" :index="item.index || ''" :key="item.index">
            <template slot="title">
              <span :class="item.icon" style="font-size:16px; margin-right: 12px"></span>
        ...
        <template v-else>
          <el-menu-item :index="item.index" :key="item.index" :noPermission="item.noPermission">
...
// import common from '../page/implementSupportCenter/context/common'
export default {
  data () {
    return {
      collapse: false,
  ...
  methods: {
    ...
    menuSelect (e, indexPath, item) {
      if (item.$attrs.noPermission) {
        return this.$message.warning('暂无该菜单权限');
      }
      this.resetAction();
      this.hideLoading();
      // let urls = '/#/' + e;
...
.topbar >>> .is-active-title .iconfont {
  color: rgb(39, 125, 218) !important;
}

.topbar >>> .is-active .iconfont {
  color: inherit;
}
.topbar >>> .iconfont {
  color: #333;
