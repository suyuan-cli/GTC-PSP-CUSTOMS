...
var distinguishBrowser = function() {
  var browser = {
      // IE浏览器
      msie: false,
      //谷歌浏览器
...
var browserr = distinguishBrowser();

export default {
  install(Vue, options = {}) {
    Vue.prototype.triggerHandleRecord = function(el) {
      api.handleRecordData({ datas: [el] }).then(res => {
        bus.$emit('handlerecordPage.refresh');
      });
    };
    //显示loading
    ...
    // 下载的公共方法
    Vue.prototype.download = function(url, functionmodule, optcontent) {
      ...
      if (functionmodule && optcontent) {
        this.triggerHandleRecord({
          functionmodule: functionmodule, // 模块名
          opttype: '06',
    ...
    Vue.prototype.formatDate = function(
      ...
      return value ? moment(Number.parseInt(value)).format(formatStr) : '';
    };
    /*给对象设置默认值*/
